...
This repository includes a GitHub Actions workflow at `.github/workflows/ci.yml` which runs linters (black, isort, ruff), executes tests with pytest, and builds a Docker image on push and pull requests to `main`/`master`.

To run the same checks locally, use the helper script `ci/run_local_ci.sh`:

```bash
chmod +x ci/run_local_ci.sh
./ci/run_local_ci.sh
```

Install pre-commit hooks locally with:
...
Run full test-suite in Docker (recommended if local installs fail):

```bash
# Build test image (may take some time on first run)
docker build -f Dockerfile.ci -t aiplatform-ci .

# Run tests inside container
docker run --rm aiplatform-ci
```
